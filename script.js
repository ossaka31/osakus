...
function updateActiveLangButton(lang) {
    ...
    // Mark active for CSS styling (color: orange)
    activeBtn.classList.add('active');

    if (!lens || !track) return;

    // Lens positioning logic is handled by layout(), but we trigger it here if needed
    // However, the resizeObserver/layout loop handles it.
}
...
function updateStaticUIText() {
    ...
    if (welcomeTitle) {
        if (isValorantPage) welcomeTitle.textContent = getTranslation('valorant_welcome_title');
        else if (isTruckersMPPage) welcomeTitle.textContent = getTranslation('truckersmp_welcome_title');
        else welcomeTitle.textContent = getTranslation('welcome_title');
    }
    if (welcomeDesc) {
        if (isValorantPage) welcomeDesc.textContent = getTranslation('valorant_welcome_desc');
        else if (isTruckersMPPage) welcomeDesc.textContent = getTranslation('truckersmp_welcome_desc');
        else welcomeDesc.textContent = getTranslation('welcome_desc');
    }
...
function renderHighlights(statsArray) {
    const grid = document.getElementById('dashboard-grid');
    if (!grid || !Array.isArray(statsArray)) return;
...
function renderMobileAccordion(statsArray) {
    const grid = document.getElementById('dashboard-grid');
    if (!grid) return;
    // ... Same logic as desktop highlights for top cards ...
    // But we focus on the accordion part here.
    ...
    const makeSection = (title, items) => {
        ...
        if (accordionState[title]) sec.classList.add('open');
...
async function executeSearch(userInput) {
    ...
    try {
      ...
      if (data && data.error) {
        ...
        try { localStorage.setItem('last_query', String(userInput)); } catch (e) {}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function createSettingsPanel() {
        ...
        try { panel.style.setProperty('z-index', '2147483647', 'important'); } catch (e) {}
        return panel;
    }
    ...
    function setSettingsOpen(open) {
        ...
        if (settingsFab) settingsFab.setAttribute('aria-pressed', window.isSettingsOpen ? 'true' : 'false');
    ...
    // Expose toggleSettings to outer scope if needed by delegation
    // But since delegation is inside DOMContentLoaded, it can access it directly.

    closeSettings();

    // Bind FAB click via Delegation as per requirement
    // First, cleanup old listeners if any (simple way is just to not add them, which we do here)
    // We add the delegated listener:
    ...
    function initLangSelector() {
        ...
        function layout() {
            ...
            // Parallax prop
            const minX = centers[0].x - (lensW / 2) + bias;
            const maxX = centers[centers.length - 1].x - (lensW / 2) + bias;
    ...
    function initNavSelector() {
        ...
        function layout() {
            ...
            if (!active) active = links[0]; // fallback
        ...
        // Same Race Condition fix for Nav
        if (window.ResizeObserver) {
            const ro = new ResizeObserver(() => layout());
            ro.observe(track);
        }
    ...
    function ensureBottomActions() {
        ...
        if (baSettings) baSettings.addEventListener('click', () => {
            toggleSettings();
            syncBottomActions();
        });
        return el;
    ...
    function syncBottomActions() {
        ...
        if (!ba) return;
        if (!isMobileUI()) { ba.hidden = true; return; }
        // If settings open, we might want to hide it, or keep it.
        // The implementation says: hidden if settings NOT open?
        // Wait, original code: ba.hidden = window.isSettingsOpen ? false : true;
        // That means bottom bar only shows when settings is open? No, that seems wrong.
        // Actually, the original code had: "if (!mobileUI) { ba.hidden = true; return; } if (!settingsPanelElement) { ba.hidden = true; return; } ba.hidden = window.isSettingsOpen ? false : true;"
        // It seems bottom actions was intended to be a replacement for something else or part of the menu.
        // I will keep it as "hidden if not mobile".
        ba.hidden = false;
    }
